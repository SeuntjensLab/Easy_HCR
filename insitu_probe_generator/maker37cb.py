def maker(name,fullseq,amplifier,pause,choose,polyAT,polyCG,BlastProbes,db,dropout,show,report,maxprobe,numbr): 
    ...
    def output(cdna,g,fullseq,count,amplifier,name,pause,seqs):
        ...
        if int(count) > 0:
            ...
            tab = ","
            string_output = "Pair#"+str(tab)+"Initiator"+str(tab)+"Spacer"+str(tab)+"Probe"+str(tab)+"Probe"+str(tab)+"Spacer"+str(tab)+"Initiator" + "\n"
            i=0
            while i < len(seqs):
                string_output += str(i+1)+str(tab)+str(upinit)+str(tab)+str(uspc)+str(tab)+str(seqs[i][1][27:52])+str(tab)+str(seqs[i][1][0:25])+str(tab)+str(dspc)+str(tab)+str(dninit) + "\n"
                i+=1
            tab = " "
    ...
    string_output = output(cdna,g,fullseq,count,amplifier,name,pause,seqs)
